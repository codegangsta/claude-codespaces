...
class AgentRequestHandler {
    ...
    async handleAgentRequest(event) {
        ...
        try {
            ...
            const codespace = await this.createAndSetupCodespace(repositoryInfo, prompt, context, issueId);
            ...
            logger_1.logger.info(`Successfully completed agent request for issue: ${issueId}`, {
                codespaceUrl: readyCodespace.web_url,
            });
        }
        catch (error) {
            logger_1.logger.error(`Failed to process agent request for issue ${issueId}:`, error);
     ...
     */
    async respondImmediately(issueId) {
        try {
            logger_1.logger.info(`Sending immediate response for issue: ${issueId}`);
            ...
            logger_1.logger.info(`Successfully sent immediate response for issue: ${issueId}`);
        }
        catch (error) {
            logger_1.logger.error(`Failed to send immediate response for issue ${issueId}:`, error);
     ...
     * Step 2: Get repository information from Linear team
     */
    async getRepositoryInfo(teamId) {
        try {
            logger_1.logger.info(`Getting repository info for team: ${teamId}`);
            const repositoryInfo = await this.linearService.getRepositoryInfoFromTeam(teamId);
            if (!repositoryInfo) {
                logger_1.logger.warn(`No repository configured for team: ${teamId}`);
                return null;
            }
            logger_1.logger.info(`Found repository: ${repositoryInfo.fullName}`, repositoryInfo);
     ...
     */
    async createAndSetupCodespace(repositoryInfo, prompt, context, issueId) {
        try {
            logger_1.logger.codespace(`Creating codespace for issue: ${issueId}`);
            ...
            const codespace = await this.githubService.createCodespace(repositoryInfo, prompt, context, issueId);
            logger_1.logger.codespace(`Successfully created codespace: ${codespace.name}`, {
            ...
            logger_1.logger.error(`Failed to create codespace for issue ${issueId}:`, error);
     ...
     * Step 4: Wait for Codespace to be ready
     */
    async waitForCodespaceReady(codespaceName) {
        try {
            ...
            logger_1.logger.error(`Failed waiting for codespace ${codespaceName}:`, error);
     ...
     */
    async attachCodespaceToIssue(issueId, codespace) {
        try {
            logger_1.logger.info(`Attaching codespace to issue: ${issueId}`);
            ...
            logger_1.logger.info(`Successfully attached codespace to issue: ${issueId}`);
        }
        catch (error) {
            logger_1.logger.error(`Failed to attach codespace to issue ${issueId}:`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async handleError(issueId, error) {
        try {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
            logger_1.logger.error(`Handling error for issue ${issueId}:`, { error: errorMessage });
            ...
            logger_1.logger.info(`Error notification sent to Linear for issue: ${issueId}`);
        }
        catch (notificationError) {
            logger_1.logger.error(`Failed to send error notification for issue ${issueId}:`, notificationError);
     ...
     */
    async getAgentProgress(issueId) {
        try {
    ...
    async healthCheck() {
        ...
        try {
            ...
            logger_1.logger.error('Linear service health check failed:', error);
        }
        try {
            ...
            logger_1.logger.error('GitHub service health check failed:', error);
        }
...
exports.AgentRequestHandler = AgentRequestHandler;
